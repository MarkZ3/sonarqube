...
public class MBeans {
  ...
  /**
   * Register a MBean into server
   */
  public static <M> void register(String name, M instance) {
    try {
      ...
      Class<M> mbeanInterface = null;
      Class<?>[] interfaces = instance.getClass().getInterfaces();
      for (Class<?> anInterface : interfaces) {
        if (anInterface.getName().endsWith("MBean")) {
          mbeanInterface = (Class<M>) anInterface;
          break;
        }
      }
  ...
  public static void unregister(String name) {
    try {
      ...
      LoggerFactory.getLogger(MBeans.class).warn("Can not unregister MBean [" + name + "]", e);
    }
  }
}
