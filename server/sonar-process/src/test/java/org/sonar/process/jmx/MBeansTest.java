...
public class MBeansTest {
  ...
  public void register_and_unregister() throws Exception {
    ...
    MBeans.register(FAKE_NAME, mbean);
    assertThat(lookupMBean()).isNotNull();

    MBeans.unregister(FAKE_NAME);
    assertThat(lookupMBean()).isNull();
  }
  ...
  public void fail_if_mbean_interface_can_not_be_found() {
    ...
    MBeans.register(FAKE_NAME, "not a mbean");
  }
  ...
  public void support_implementation_in_different_package_than_interface() throws Exception {
    ...
    MBeans.register(FAKE_NAME, new org.sonar.process.jmx.test.Fake());
    assertThat(lookupMBean()).isNotNull();
  ...
  public void only_statics() {
    assertThat(TestUtils.hasOnlyPrivateConstructors(MBeans.class)).isTrue();
  }
